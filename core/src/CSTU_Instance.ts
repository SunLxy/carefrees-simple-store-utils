...
export class CSTU_Instance {
  ...
  */
   ...
   * @param field 字段
   * */
  _get_map = (field: string): Map<Symbol | Object, CSTU_SelectorListItemType> => {
   ...
   * @param props 内部操作参数
   * */
  _create_base_register = <R = any>(componentField: string, storeField: string, initialField: string, props: CSTU_RegisterProps) => {
    this._get_list<CSTU_RegisterProps>(componentField).push(props)
    return () => {
      this[componentField] = this._get_list<CSTU_RegisterProps>(componentField).filter((ite) => ite !== props)
      const { preserve = true } = props
      const currentPath = getFormatPath(props.path)
      /**查询是否存在相同字段的组件*/
      const finx = this._get_list<CSTU_RegisterProps>(componentField).find((item) => getFormatPath(item.path) === currentPath)
      /**当不存储 并且 没有相同字段组件的时候*/
      if (!preserve && !finx) {
        /**
        ...
        * */
        ...
        const defaultValue = CSTU_getValue(this._get_store<R>(initialField), pathArr)
        const value = CSTU_getValue(this._get_store<R>(storeField), pathArr)
        if (defaultValue !== value) {
          this[storeField] = CSTU_setValue(this._get_store<R>(storeField), pathArr, defaultValue)
        }
      }
    }
   ...
   * @param props 内部操作参数
   * */
  _create_base_registerWatch = (watchField: string, props: CSTU_RegisterWatchProps) => {
    this._get_list<CSTU_RegisterWatchProps>(watchField).push(props)
    return () => {
      this[watchField] = this._get_list<CSTU_RegisterWatchProps>(watchField).filter((ite) => ite !== props)
    }
  }
  ...
  _create_base_notice = (componentField: string, path: PathTypes,) => {
    ...
    const componentList = this._get_list<CSTU_RegisterProps>(componentField).filter((item) => getFormatPath(item.path) === newPath)
  ...
  */
  _create_base_bathNotice = (componentField: string, paths: string[] | boolean = true) => {
    if (Array.isArray(paths)) {
      ...
      // 更新所有组件
      this._get_list<CSTU_RegisterProps>(componentField).forEach((component) => {
        /**通知更新组件*/
  ...
  _create_base_noticeWatch = (watchField: string, storeField: string, path: PathTypes,) => {
    ...
    const value = CSTU_getValue(this._get_store(storeField), toArray(path))
    this._get_list<CSTU_RegisterWatchProps>(watchField).forEach((item) => {
  ...
  _create_base_updateValue = <K = any>(
    ...
    const preVaue = CSTU_getValue(this._get_store(storeField), toArray(path));
    this[storeField] = CSTU_setValue(this._get_store(storeField), toArray(path), value);
  ...
  _create_base_bathUpdateValue = (
    ...
    if (values) {
      Object.entries(values).forEach(([path, value]) => {
        const preVaue = CSTU_getValue(this._get_store(storeField), toArray(path));
        this[storeField] = CSTU_setValue(this._get_store(storeField), splitPath(path), value)
        /**判断值相等 , 当相等的时候才进行更新监听的值*/
        if (preVaue !== value)
   ...
   * @param path 获取数据的路径
   * */
  _create_base_CSTU_getValue = (storeField: string, path?: PathTypes) => {
    if (path) {
      return CSTU_getValue(this._get_store(storeField), toArray(path))
    }
  ...
  */
  _create_base_init = <T = any>(storeField: string, initialField: string, initialValue?: T) => {
    ...
    this[storeField] = CSTU_merge(this._get_store(initialField), this._get_store(storeField))
  }
  ...
  */
  _create_base_registerSelector = <Selected = unknown, T extends CSTU_Instance = CSTU_Instance>(
